...
//! [`LinuxWalker`](crate::LinuxWalker) so that `examples/walkbench.rs` can
...
use crate::{LinuxWalker, Options as ScanOptions, Stat, Walked, allocated_from_blocks};
use dirstats_scan::Kind;
...
impl Walk {
    ...
    pub(crate) fn start(root: &Path, options: &ScanOptions) -> Option<Self> {
        let tuning = options.walker.clone();
    ...
    pub(crate) fn next(&mut self, cancel: &AtomicBool) -> Option<io::Result<Walked>> {
        loop {
...
fn stat_of(stat: &Statx) -> Stat {
    Stat {
        ...
        allocated_size: allocated_from_blocks(stat.size, stat.blocks, u64::from(stat.blksize)),
...
mod tests {
    ...
    #[test]
    fn every_variant_stops_on_cancel() {
        ...
        for (name, walker) in LinuxWalker::variants() {
            ...
            let options = ScanOptions { walker, ..ScanOptions::default() };
            let mut walk = Walk::start(dir.path(), &options).unwrap();
