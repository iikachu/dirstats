...
fn main() {
    let args = args();
    // `None` is the baseline, dirstats-scan as it ships.
    let variants: Vec<(&str, Option<LinuxWalker>)> = std::iter::once(("dua-core (std::fs)", None))
        .chain(LinuxWalker::variants().into_iter().map(|(name, walker)| (name, Some(walker))))
        .enumerate()
        .filter(|(i, (name, _))| {
    ...
    for round in 0..args.reps {
        for step in 0..variants.len() {
            ...
            let (name, walker) = &variants[index];
            if args.cold {
                drop_caches();
            }
